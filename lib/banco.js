...
function paraUsuario(linha) {
  ...
  return {
    ...
    email: linha.email,
    // O resumo do código de recuperação, nunca o código em si — ele só existe
    // em texto no instante em que é gerado e mostrado na tela.
    recuperacao: linha.recuperacao,
    recuperacaoEm: iso(linha.recuperacao_em)
  };
}
 ...
 *
 * O e-mail repetido não passa por aqui: ele estoura o índice único e vira uma
 * exceção, que a rota traduz separadamente. São dois conflitos diferentes e a
 * pessoa precisa saber qual dos dois aconteceu.
 */
async function criarUsuario(usuario) {
  const linha = await uma(
    `insert into usuarios (id, nome, chave, senha, papel, email)
     values ($1, $2, $3, $4, $5, $6::text)
     on conflict (chave) do nothing
     returning *`,
    [usuario.id, usuario.nome, usuario.chave, usuario.senha, usuario.papel, usuario.email || null]
  );
...
async function contarEspremedores() {
  const linha = await uma("select count(*)::int as total from usuarios where papel = 'espremedor'");
  return linha.total;
}

/**
 * Só os nomes de quem é espremedor.
 *
 * Serve à tela de "perdi o código de recuperação", que precisa dizer a quem
 * pedir. Vai sem login, porque quem esqueceu a senha justamente não consegue
 * entrar — e nome não é segredo aqui: o cadastro é aberto, então qualquer um
 * criaria um acesso e veria a lista inteira de gente de qualquer jeito.
 */
async function listarEspremedores() {
  const { rows } = await consulta(
    "select nome from usuarios where papel = 'espremedor' order by nome"
  );
  return rows.map((linha) => linha.nome);
}

/** Acha a pessoa pelo e-mail, sem diferenciar maiúscula de minúscula. */
async function usuarioPorEmail(email) {
  return paraUsuario(await uma('select * from usuarios where lower(email) = lower($1)', [email]));
}

/**
 * Grava o e-mail (ou tira, com null).
 *
 * Devolve null quando o endereço já pertence a outra pessoa — é o índice único
 * que barra, e a rota transforma isso num 409 explicando.
 */
async function gravarEmail(id, email) {
  try {
    return paraUsuario(
      await uma('update usuarios set email = $2::text where id = $1 returning *', [id, email || null])
    );
  } catch (falha) {
    // 23505 = violação de unicidade. Qualquer outro erro é problema de verdade
    // e precisa continuar subindo.
    if (falha.code === '23505') return null;
    throw falha;
  }
}
...
/**
 * Guarda um pedido de redefinição.
 *
 * O `resumo` é o SHA-256 do token que foi no link, nunca o token em si. Os
 * pedidos anteriores da mesma pessoa são marcados como usados: pedir um link
 * novo tem que invalidar o antigo, senão um e-mail velho na caixa de entrada
 * continuaria abrindo a conta.
 */
async function criarRedefinicao(resumo, usuarioId) {
  const conexao = await pool.connect();
  try {
    await conexao.query('begin');
    await conexao.query(
      'update redefinicoes set usada_em = now() where usuario_id = $1 and usada_em is null',
      [usuarioId]
    );
    await conexao.query('insert into redefinicoes (token, usuario_id) values ($1, $2)', [resumo, usuarioId]);
...
/** O pedido de redefinição ainda válido, com os dados de quem pediu. */
async function redefinicaoValida(resumo, duracaoMs) {
  const linha = await uma(
    `select r.token, r.usuario_id, u.nome, u.papel, u.email
       from redefinicoes r
       join usuarios u on u.id = r.usuario_id
      where r.token = $1 and r.usada_em is null and r.criada_em > $2`,
    [resumo, new Date(Date.now() - duracaoMs)]
  );
  if (!linha) return null;
  return {
    usuarioId: linha.usuario_id,
...
/** Marca o link como gasto. Devolve false se outra requisição chegou primeiro. */
async function gastarRedefinicao(resumo) {
  const linha = await uma(
    'update redefinicoes set usada_em = now() where token = $1 and usada_em is null returning token',
    [resumo]
  );
  return Boolean(linha);
}
...
/**
 * Marca como usados os links de redefinição que uma pessoa ainda tem em aberto.
 *
 * Trocar o e-mail precisa invalidar o que já foi enviado: o link antigo está na
 * caixa de entrada do endereço anterior, e continuar valendo deixaria a conta
 * aberta para quem não é mais o dono do e-mail.
 */
async function invalidarRedefinicoes(usuarioId) {
  await consulta(
    'update redefinicoes set usada_em = now() where usuario_id = $1 and usada_em is null',
    [usuarioId]
  );
...
/** Some com os pedidos vencidos. Chamado na partida, junto das sessões. */
async function limparRedefinicoesVencidas(duracaoMs) {
  await consulta('delete from redefinicoes where criada_em < $1', [new Date(Date.now() - duracaoMs)]);
}

/** Guarda o resumo do código de recuperação (ou tira, com null). */
async function gravarRecuperacao(id, resumo) {
  return paraUsuario(
    await uma(
      `update usuarios
          set recuperacao = $2::text,
              recuperacao_em = $3::timestamptz
        where id = $1
      returning *`,
      [id, resumo || null, resumo ? new Date() : null]
    )
  );
}
 ...
 *
 * Chamado quando a senha muda. Quem redefiniu a senha porque perdeu o acesso
 * precisa que os aparelhos antigos parem de valer — inclusive um que estivesse
 * na mão de outra pessoa, que é metade do motivo de trocar a senha.
 */
async function fecharSessoesDoUsuario(usuarioId) {
  await consulta('delete from sessoes where usuario_id = $1', [usuarioId]);
}
...
async function atualizarProduto(id, mudancas) {
  ...
  // Preço corrigido na mão pelo espremedor também é história de preço — senão o
...
/** Quem ficou devendo em rodadas já fechadas — a lista de cobrança do espremedor. */
...
/** O espremedor confirmando que o dinheiro caiu. */
...
module.exports = {
  ...
  redefinicaoValida,
  gastarRedefinicao,
  invalidarRedefinicoes,
  limparRedefinicoesVencidas,
  gravarRecuperacao,
  abrirSessao,
  usuarioPorToken,
  fecharSessao,
  fecharSessoesDoUsuario,
