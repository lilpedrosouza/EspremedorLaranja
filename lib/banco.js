...
/**
 * Guarda o preço do produto, mas só se ele for diferente do último guardado.
 *
 * O `where` faz essa conferência dentro do próprio insert: assim a leitura do
 * site, que roda de 12 em 12 horas e quase sempre traz o mesmo preço, não deixa
 * duas linhas iguais por dia na tabela. O que interessa é a mudança — cada linha
 * vale até a próxima.
 *
 * O `executor` existe para esta gravação poder entrar na mesma transação da
 * gravação do catálogo. Sem ele, vale o pool.
 */
const SQL_REGISTRAR_PRECO = `
  insert into precos (produto_id, preco)
  select $1::text, $2::numeric
   where coalesce(
           (select p.preco
              from precos p
             where p.produto_id = $1::text
...
async function registrarPreco(produtoId, preco, executor = pool) {
  ...
  await executor.query(SQL_REGISTRAR_PRECO, [produtoId, valor]);
}
...
async function historicoDoProduto(produtoId, desde) {
  ...
  return rows.map((linha) => ({ preco: Number(linha.preco), em: iso(linha.registrado_em) }));
}
...
async function historicoDeTodos(desde) {
  const { rows } = await consulta(
    `select produto_id, preco, registrado_em from precos
      ...
      order by produto_id, registrado_em`,
    [desde]
  );
...
async function salvarCatalogo(catalogo) {
  ...
  try {
    ...
    for (const produto of catalogo) {
      ...
      await registrarPreco(produto.id, produto.preco, conexao);
    }
